...
const deploy = {
    command:
        "deploy [accountId] [wasmFile] [initFunction] [initArgs] [initGas] [initDeposit]",
    // command: 'deploy',
...
yargs // eslint-disable-line
    ...
    .command(
        require("../commands/create-account").createAccountCommandDeprecated
    )
    .command(viewAccount)
    ...
    .command(require("../commands/clean-state"))
    .command(sendMoney)
    .command(clean)
    .command(stake)
    .command(login)
    ...
    .epilogue(
        chalk`Check out our epic whiteboard series: {bold http://near.ai/wbs}`
    )
    .wrap(null).argv;
